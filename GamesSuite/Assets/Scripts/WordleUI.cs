...
public class WordleUI : MonoBehaviour
{
    ...
    public void changeBlockColor() {
        ...
        for (int i = 0; i < playerInputWord.Length; i++) {
            char currLetter = wordlePlayer.correctWord[i]; // current letter of the correct word to compare it to the player's guess
            ...
            if (currLetter != playerInputWord[i] && playerInputWord.Contains(currLetter)) { // if player's letter is in correct word but wrong position
